...
def calc_local_integral_value(src, left_top, right_bottom):
    ...
    # 실습 설명을 들으면 보다 쉽게 구현 가능
    ##########################################################################
    (left_h, left_w) = left_top
    (right_h, right_w) = right_bottom
    if left_h == 0:
        left_h = 1
    if left_w == 0:
        left_w = 1
    array_sum = src[left_h - 1, left_w - 1] + src[right_h, right_w] - src[left_h - 1, right_w] - src[right_h, left_w - 1]
    return array_sum
...
def main():
    # src = cv2.imread('../../img/zebra.png') # shape : (552, 435, 3)
    ...
    cv2.imshow('harris_img ' + '201802101' , harris_img)
    cv2.imshow('harris_integral_img ' + '201802101' , harris_integral_img)
    cv2.waitKey()
