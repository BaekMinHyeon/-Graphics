    ...
    for i in range(len(keypoints)):
        ...
        for row in range(-8, 8):
            for col in range(-8, 8):
                ...
                angle_index = int(angle[p_y, p_x]//10.)
                orient_hist[angle_index] += magnitude[p_y, p_x]*gaussian_weight
        ###################################################################
        ## ToDo
        ...
        for j in range(len(orient_hist)):
            if((orient_hist[j] > 0.8 * np.max(orient_hist)) and (j != np.argmax(orient_hist))):
                keypoints.append(cv2.KeyPoint(x, y, keypoints[i].size, j*10, keypoints[i].response, keypoints[i].octave, keypoints[i].class_id))
    ...
    for i in range(len(keypoints)):
        ...
        for row in range(-8, 8):
            for col in range(-8, 8):
                ...
                weight = gaussian_weight # 위에서 활용한 식을 이용
                k = ((row+8)//4)*4+((col+8)//4)
                descriptors[i, descriptor_angle + 8 * k] += weight * magnitude[p_y, p_x]
...
def main():
    ...
    # 결과의 학번 작성하기!
    cv2.imshow('201802101_my_sift', result)
    cv2.waitKey()
